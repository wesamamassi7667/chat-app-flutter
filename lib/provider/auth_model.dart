...
class AuthProvider extends ChangeNotifier {
  ...
  AuthProvider({required this.googleSignIn,required this.firebaseFirestore,required this.firebaseAuth});
  ...
  Future<bool> signWithGoogle() async{
    ...
    if(googleUser!=null){
      ...
      if(firebaseUser!=null){
        final result=await firebaseFirestore.collection(userCollection).where(
          "id",isEqualTo:firebaseUser.uid
        ).get();
        final docs=result.docs;
        if(docs.length==0){
         //create new user
          firebaseFirestore.collection(userCollection).doc(firebaseUser.uid).set(
            {
